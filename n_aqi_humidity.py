...
if __name__ == "__main__":
    con = db_connect()
    cur = con.cursor()
    query1 = "CREATE TABLE IF NOT EXISTS aqi(pm1 real, pm2 real, humidity real, temperature real, time Date DEFAULT (datetime('now', 'localtime')) )"
    try:
        cur.execute(query1)
    except:
        "Failed to create Table"
    query2 = "INSERT INTO aqi (pm1,pm2,humidity,temperature) VALUES (?,?,?,?)"
    ...
    while True:
        ...
        try:            
            cur.execute(query2,(pm25avg,pm10avg, humidity, temperature))
            time.sleep(1)
            con.commit()
        except:
